        ...
        with suggest_col:
            ...
            <div class="ai-card">
                ...
                <div class="ai-card-body">
                    <div style="font-size:22px;margin:8px 0">{suggestion.get('icon','🤖')} <b style="color:#5850EC">{suggestion.get('label','Unknown')}</b></div>
                    {suggestion.get('reason','')}
                    <div style="margin-top:10px">
                        <span class="badge badge-blue">Model: {suggestion.get('model','')}</span>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if st.session_state.get("clean_log"):
            with st.expander("✔ Auto-Clean Report", expanded=True):
                for entry in st.session_state["clean_log"]:
                    st.markdown(f"- {entry}")
            if st.button("↺ Undo Cleaning", key=f"undo_clean_{k}"):
                st.session_state.pop("cleaned_df", None)
                st.session_state.pop("clean_log", None)
                st.rerun()
...
with tab_resume:
    ...
    if not prev_uploads:
        ...
        for u in prev_uploads:
            ...
            with col3:
                if st.button("✖", key=f"del_{u['id']}", help="Delete this file"):
                    try:
                        if os.path.exists(u["file_path"]):
                    ...
                    st.success(f"✖ Deleted **{u['filename']}**")
                    st.rerun()
...
(function(){
  function scrollToAnchor(){
    var el = window.parent.document.getElementById('analysis-anchor');
    if(el){ el.scrollIntoView({behavior:'smooth', block:'start'}); }
    else { setTimeout(scrollToAnchor, 200); }
  }
