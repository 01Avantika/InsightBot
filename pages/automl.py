...
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"] {
    ...
    padding: .42rem .85rem !important; border-radius: 7px !important;
    margin: 1px 7px !important; font-size: .77rem !important; font-weight: 500 !important;
    color: var(--text2) !important; text-decoration: none !important;
    font-family: var(--sans) !important; transition: all .15s !important;
}
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"] * { color: var(--text2) !important; }
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"]:hover,
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"]:hover * {
    color: var(--purple-l) !important; background: rgba(124,58,237,.1) !important;
}
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"][aria-current="page"],
[data-testid="stSidebar"] a[data-testid="stPageLink-NavLink"][aria-current="page"] * {
    color: var(--purple-l) !important; background: rgba(124,58,237,.15) !important; font-weight:600 !important;
}
[data-testid="stSidebar"] h3 {
    font-size: .55rem !important; font-weight: 700 !important;
    letter-spacing: .13em !important; text-transform: uppercase !important;
    color: var(--text3) !important; padding: 1rem .85rem .3rem !important; margin: 0 !important;
}
[data-testid="stSidebar"] hr { border-color: var(--border) !important; margin: .35rem .55rem !important; }
...
.sb-logo2 {
    ...
    padding:1rem .85rem .75rem; border-bottom:1px solid var(--border); margin-bottom:.25rem;
}
.sb-logo-icon2 {
...
.sb-logo-name2 { font-size:.88rem; font-weight:700; color:var(--text); font-family:var(--sans); }
.sb-logo-sub2  { font-size:.49rem; color:var(--text3); letter-spacing:.12em; text-transform:uppercase; font-family:var(--mono); }
.sb-user2 {
    display:flex; align-items:center; gap:8px; padding:.55rem .75rem;
    border-radius:9px; background:rgba(124,58,237,.08);
    border:1px solid rgba(124,58,237,.14); margin:0 .45rem .45rem;
}
.sb-av2 {
...
.sb-nm2 { font-size:.72rem; font-weight:600; color:var(--text); }
.sb-em2 { font-size:.6rem; color:var(--text3); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:118px; }
</style>
...
with st.sidebar:
    st.markdown(f"""
    <div class="sb-logo2">
        <div class="sb-logo-icon2">🤖</div>
        <div>
            <div class="sb-logo-name2">InsightBot</div>
            <div class="sb-logo-sub2">AI Intelligence</div>
        </div>
    </div>""", unsafe_allow_html=True)

    st.markdown("### Navigation")
    st.page_link("pages/dashboard.py", label="⊞ Dashboard")
    st.page_link("pages/analyze.py",   label="☁ Upload")
    st.page_link("pages/automl.py",    label="⚙ AutoML")
    st.page_link("pages/chat.py",      label="🗨 Chat")
    st.page_link("pages/history.py",   label="↺ History")

    if df is not None:
        st.markdown("<hr style='border-color:rgba(255,255,255,.06);margin:.35rem .4rem'/>",
                    unsafe_allow_html=True)
        st.markdown(f"""
        <div style='padding:.45rem .85rem;font-size:.64rem;font-family:var(--mono);color:var(--text2)'>
            <div style='color:var(--text3);letter-spacing:.1em;text-transform:uppercase;
                 margin-bottom:.25rem;font-size:.53rem'>Current Plan</div>
            <div style='font-weight:700;color:var(--text)'>Enterprise Suite</div>
        </div>""", unsafe_allow_html=True)

    for _ in range(5): st.markdown("")
    st.markdown("<hr style='border-color:rgba(255,255,255,.06);margin:.35rem .4rem'/>",
                unsafe_allow_html=True)
    st.markdown(f"""
    <div class="sb-user2">
        <div class="sb-av2">{u_initial}</div>
        <div style="min-width:0">
            <div class="sb-nm2">{u_name}</div>
            <div class="sb-em2">{u_email}</div>
        </div>
    </div>""", unsafe_allow_html=True)
